...
Hệ thống được thiết kế sử dụng thuật toán AES-256-GCM qua Web Crypto API, đảm bảo dữ liệu luôn được mã hóa tại trình duyệt người dùng trước khi gửi đến máy chủ. Điểm nổi bật là khóa giải mã được nhúng vào URL fragment, giúp ngăn chặn triệt để việc máy chủ tiếp cận được nội dung bản rõ (plaintext). Phía Backend sử dụng Golang kết hợp với Redis để xử lý logic lưu trữ có thời hạn (TTL) và đảm bảo tính nguyên tử (atomic) để bí mật chỉ có thể được đọc một lần duy nhất.

Kết quả triển khai hệ thống cho thấy Secret Letter không chỉ đáp ứng tốt các yêu cầu về an toàn thông tin mà còn duy trì hiệu năng cao, khả năng chịu tải tốt và chống lại các bot cào dữ liệu qua cơ chế Reveal Gate. Hệ thống sẵn sàng làm nền tảng cho các nghiệp vụ trao đổi thông tin nhạy cảm của cá nhân và doanh nghiệp. Mã nguồn mở của hệ thống được công bố công khai tại kho lưu trữ GitHub \url{https://github.com/vmcchooky/secret-letter} và ứng dụng đang được vận hành thực tế tại \url{https://secret.quorxi.io.vn}.
...
The system is designed using the AES-256-GCM algorithm via the Web Crypto API, ensuring that data is always encrypted in the user's browser before being sent to the server. A key feature is that the decryption key is embedded in the URL fragment, completely preventing the server from accessing the plaintext content. The Backend uses Golang combined with Redis to handle time-to-live (TTL) expiration mechanisms and ensure atomic operations so that a secret can only be read exactly once.

Deployment results show that Secret Letter not only meets strict information security requirements but also maintains high performance, handles concurrent loads effectively, and protects against web crawlers through a Reveal Gate mechanism. The system is well-prepared to serve as a foundation for sensitive information exchange. The system's open-source code is publicly available on GitHub at \url{https://github.com/vmcchooky/secret-letter}, and the live application is currently deployed at \url{https://secret.quorxi.io.vn}.
\end{english}
...
\input{CHƯƠNG 2/2.1.MatMaHocUngDung}
\input{CHƯƠNG 2/2.2.KienTrucClientSideEncryption}
...
\input{CHƯƠNG 3/3.3.ThietKeLuongDuLieu}
\input{CHƯƠNG 3/3.4.ThietKeAPI}
\chapter{TRIỂN KHAI CHI TIẾT}
\input{CHƯƠNG 4/4.1.CauTrucMaNguon}
\input{CHƯƠNG 4/4.2.TrienKhaiFrontend}
\input{CHƯƠNG 4/4.3.TrienKhaiBackend}
\input{CHƯƠNG 4/4.4.GiaiQuyetBaiToanKyThuat}
\chapter{ĐÁNH GIÁ BẢO MẬT, KIỂM THỬ VÀ VẬN HÀNH}
\input{CHƯƠNG 5/5.1.MoHinhDeDoa.tex}
\input{CHƯƠNG 5/5.2.ChienLuocKiemThu}
\input{CHƯƠNG 5/5.3.DanhGiaHieuNang}
\input{CHƯƠNG 5/5.4.QuyTrinhCICD}
\newpage

\chapter*{KẾT LUẬN}
\addcontentsline{toc}{chapter}{KẾT LUẬN}

Dự án \textit{Secret Letter} đã hiện thực hóa thành công mục tiêu xây dựng một nền tảng chia sẻ thông tin nhạy cảm dựa trên kiến trúc Zero-Knowledge và mã hóa đầu-cuối tại trình duyệt (Client-side Encryption) thông qua thuật toán AES-256-GCM. Hệ thống đã giải quyết trọn vẹn bài toán bảo vệ khóa giải mã trên đường truyền mạng nhờ cơ chế URL Fragment, đồng thời hạn chế sự phơi nhiễm dữ liệu trước các kịch bản cào dữ liệu tự động (Preview Bots) bằng quy trình tương tác Reveal Gate. Nhờ đó, một vòng đời tiêu hủy dữ liệu an toàn (Burn-after-reading) được thiết lập và duy trì xuyên suốt.

Trên phương diện kỹ thuật, kiến trúc kết hợp giữa Golang và cơ sở dữ liệu in-memory Redis không chỉ đảm bảo tính nguyên tử (Atomic) thông qua Lua Script, ngăn chặn hiệu quả rủi ro tương tranh (Race Condition), mà còn cung cấp khả năng chịu tải ấn tượng. Kết quả đo đạc thực tế minh chứng hệ thống có thể xử lý thông lượng vượt mốc 3,500 RPS với độ trễ tối thiểu tiệm cận hằng số $O(1)$. Kết hợp cùng luồng kiểm thử tự động (CI/CD) và End-to-End (E2E), dự án đã đáp ứng các tiêu chuẩn khắt khe để sẵn sàng vận hành trên môi trường thực tế (Production-ready).

Về định hướng phát triển trong tương lai, dự án hướng tới việc mở rộng khả năng mã hóa cho các tệp tin đính kèm (File Attachment Encryption) dung lượng lớn thông qua các thuật toán mã hóa luồng (Stream Cipher) kết hợp Web Crypto API. Đồng thời, việc nghiên cứu áp dụng các cơ chế xác thực đa yếu tố bằng khóa phần cứng (FIDO2/WebAuthn) sẽ được tiến hành nhằm tăng cường lớp phòng thủ đối với các hành vi truy cập trái phép, củng cố vững chắc ranh giới bảo mật Zero-Knowledge mà không phá vỡ tính ẩn danh của người sử dụng.

Toàn bộ mã nguồn, tài liệu API và các kịch bản triển khai (CI/CD) của dự án được đóng gói và phát hành mã nguồn mở tại \url{https://github.com/vmcchooky/secret-letter}. Hệ thống hiện đang được triển khai thử nghiệm để người dùng trải nghiệm thực tế tại \url{https://secret.quorxi.io.vn}.
...
% \input{PHỤ LỤC/A_CodeCotLoi}
% \newpage
% \chapter{CẤU TRÚC MỘT VÀI GÓI TIN}
% \input{PHỤ LỤC/B_CauTrucMotVaiGoiTin}
