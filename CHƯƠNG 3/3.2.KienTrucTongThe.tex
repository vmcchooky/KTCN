...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textit{Lớp Gateway \& Middleware:} Đóng vai trò kiểm duyệt biên nội bộ. Trong mô hình thực tế, Caddy đóng vai trò Termination TLS (giải mã HTTPS), sau đó đẩy Request về Go Application. Bộ Middleware nội tại của Go sẽ tự đảm nhiệm việc gán định danh truy vết, thiết lập chính sách CORS và thực thi cơ chế giới hạn lưu lượng (Rate Limiting) dựa trên IP ảo đã được Caddy xác thực nhằm giảm thiểu tác động từ các đợt tấn công cạn kiệt tài nguyên.
