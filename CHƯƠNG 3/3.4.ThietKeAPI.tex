...
Để thu hẹp các bề mặt tấn công từ lớp mạng, các luồng dữ liệu HTTP trước khi chạm đến các bộ xử lý logic (Handlers) thường được đi qua một chuỗi Middleware phòng thủ theo chiều sâu (Defense in Depth). Kiến trúc này được định nghĩa trực tiếp tại mã nguồn \texttt{internal/httpapi/server.go}:
...
\end{tikzpicture}
    \caption[Luồng xử lý yêu cầu đi qua hệ thống Middleware]{Luồng xử lý yêu cầu đi qua hệ thống Middleware của API Gateway}
...
\begin{itemize}
    ...
    \item \textbf{\texttt{POST /api/secrets/:id/open} (hoặc \texttt{/consume}):} Kích hoạt chuỗi lệnh Lua Script trên Redis để trả về nguyên trạng bản mã, và ngay lập tức thực thi lệnh xóa (DEL) bản ghi một cách nguyên tử. Yêu cầu truyền vào Header \texttt{X-Reveal-Session} từ bước số 2 để chứng thực tính hợp lệ. Giao dịch thành công trả về \texttt{200 OK} kèm dữ liệu; nếu thất bại do thư đã bị tiêu hủy, trả về \texttt{410 Gone} kèm thông báo chuẩn hóa.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption[Sơ đồ trạng thái minh họa vòng đời lưu trữ thông điệp]{Sơ đồ trạng thái minh họa vòng đời lưu trữ của một thông điệp}
