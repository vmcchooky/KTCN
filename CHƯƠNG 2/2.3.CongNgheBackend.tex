...
\begin{itemize}
    ...
    \item \textbf{Kiểm soát lưu lượng (Rate Limiting):} Hệ thống duy trì một bộ định mức luồng (rate limiter) vận hành trực tiếp trên Redis. Bằng cách trích xuất an toàn địa chỉ IP thực của thiết bị đầu cuối qua tiêu đề \texttt{X-Forwarded-For} từ các dải proxy được ủy thác (trusted proxies), máy chủ thiết lập hạn mức kết nối riêng biệt cho từng loại giao dịch (tạo mới, truy xuất, kiểm tra trạng thái), từ đó hạn chế đáng kể khả năng dò quét (brute-force) mã thông điệp. Tiêu đề \texttt{X-Forwarded-For} chỉ được Backend tin cậy sau khi đã bị API Gateway (Caddy) đè/chỉnh sửa (overwrite) một cách an toàn, nhằm chống lại kỹ thuật giả mạo IP (IP Spoofing) từ tin tặc.
\end{itemize}